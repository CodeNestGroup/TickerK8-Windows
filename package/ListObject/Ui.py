...
def ListObjectReloadStyle(self):
    m = open(f'{self.Path}/assets/CSS/ListObjectMain.css').read()
    c = open(f'{self.Path}/assets/CSS/ListObject{self.Theme}.css').read()
...
def NullDataRetranslate(self):
    t = json.load(open(f'{self.Path}/assets/JSON/ListObjectNullDataTranslate.json', 'r', encoding='utf-8'))
    l = self.Language
